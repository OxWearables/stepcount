...
def summarize_cadence(
    ...
    def _cadence_max(x, steptol, min_walk_per_day=30, n=1):
        y = x[x >= steptol]
    ...
    def _cadence_p95(x, steptol, min_walk_per_day=30):
        y = x[x >= steptol]
        # if not enough walking time, return NA.
        # note: min_walk_per_day in minutes, x must be minutely
        if len(y) < min_walk_per_day:
    ...
    daily_cadence_peak1 = minutely.resample('D').agg(_cadence_max, steptol=steptol_in_minutes, min_walk_per_day=peak1_min_walk_per_day, n=1).rename('CadencePeak1(steps/min)')
    daily_cadence_peak30 = minutely.resample('D').agg(_cadence_max, steptol=steptol_in_minutes, min_walk_per_day=peak30_min_walk_per_day, n=30).rename('CadencePeak30(steps/min)')
    daily_cadence_p95 = minutely.resample('D').agg(_cadence_p95, min_walk_per_day=p95_min_walk_per_day, steptol=steptol_in_minutes).rename('Cadence95th(steps/min)')
